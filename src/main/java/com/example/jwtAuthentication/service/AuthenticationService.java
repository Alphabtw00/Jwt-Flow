...
public class AuthenticationService {
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest request){
        var authenticationResponse = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(request.getUsername(), request.getPassword())); // constructor used for un-authenticated requests without roles, just to check if credentials are valid and to match from database. If one of the AuthenticationProviders successfully authenticates the user, the AuthenticationManager (via ProviderManager) takes info from database and makes a full UserPasswordToken with roles and sets it in security context itself. Used for unverified credentials.  //todo add try catch and then controller advice
